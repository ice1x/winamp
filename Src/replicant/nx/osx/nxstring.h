/*
 ...
 macOS implementation of the nx string layer.

 Counterpart to win/nxstring.h. The Win32 reference stores text as `wchar_t`,
 which is a 16-bit UTF-16 code unit on Windows. macOS `wchar_t` is 32-bit
 (UTF-32), so this port stores text internally as explicit 16-bit UTF-16 code
 units (`nx_utf16_t`, matching foundation/osx-arm64/types.h `ns_char_t`). This
 keeps the on-disk / over-the-wire binary contract identical to Windows
 (nx_charset_utf16le is the raw internal buffer) while still letting the public
 entry points accept the platform-native `wchar_t`.

 Differences from the Win32 header:
   - no <windows.h>; integer/GUID types come from foundation/types.h.
   - the heap handle is an opaque `void *` (there is no Win32 HANDLE / process
     heap on macOS); allocation goes through malloc/realloc/free and the handle
     argument is ignored. The functions are kept for source compatibility.
   - charset code-page numbers match the Win32 values (CP_ACP == 0,
     CP_UTF8 == 65001) so cross-platform callers behave identically.
   - the `wchar_t`-taking constructors treat their argument as a native (UTF-32)
     wide string and transcode it to the internal UTF-16 representation.
...
extern "C" {
	...
	/* 16-bit UTF-16 code unit — the internal storage element. On Windows this is
	   wchar_t; on macOS wchar_t is 32-bit so we pin it explicitly. */
	typedef uint16_t nx_utf16_t;

	typedef enum
	{
		nx_charset_ascii = 20127,
		nx_charset_latin1 = 28591,
		nx_charset_system = 0,     /* CP_ACP on Win32; UTF-8 system locale on macOS */
		nx_charset_utf8 = 65001,   /* CP_UTF8 */
	...
	{
		size_t ref_count;
		size_t len;          /* length in UTF-16 code units, excluding terminator */
		nx_utf16_t string[1]; /* utf-16, NUL terminated */
	...
	NX_API int NXStringSetHeap(void *string_heap);
	...
	NX_API nx_string_t NXStringCreateWithHeap(void *heap, const wchar_t *str);
	NX_API nx_string_t NXStringMallocWithHeap(void *heap, size_t characters);
	...
	/* creates an NXString with the base path from the passed in filename (retains an appended separator) */
